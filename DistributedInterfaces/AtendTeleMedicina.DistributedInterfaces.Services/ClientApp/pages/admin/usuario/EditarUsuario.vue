<template>
  <div>
    <el-row>
      <el-col :span="14">
        <h2 class="box-card--h2">Editar Usuario</h2>
      </el-col>
    ...
    <el-row>
      <el-form :model="formUsuario" status-icon :rules="validacoes" ref="formUsuario" label-width="120px" label-position="top" class="forms--usuario">
        <el-row :gutter="24">
          ...
          <el-col :xs="24" :sm="24" :md="24" :lg="5" :xl="5">
            <el-form-item label="Senha" class="forms--checkbox-right" prop="senha">
              <el-checkbox v-model="alterarSenha" @change="onAlterarSenha">Alterar Senha</el-checkbox>
              <el-input v-model="formUsuario.senha" :disabled="alterarSenha == false" type="password" minlength="8" maxlength="16" placeholder="Digite a senha" />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="24" :lg="5" :xl="5">
            <el-form-item label="Confirmação" prop="senhaConfirmacao">
              <el-input v-model="formUsuario.senhaConfirmacao" :disabled="alterarSenha == false" type="password" minlength="8" maxlength="16" placeholder="Confirme a senha digitada"/>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="24">
          ...
          <el-col :xs="24" :sm="24" :md="24" :lg="5" :xl="5">
            <el-form-item label="Cidade" prop="cidadeId">
              <el-select filterable placeholder="Selecione a Cidade" v-model="formUsuario.cidadeId" v-loading.body="loading.cidades">
        ...
        <el-row :gutter="24">
          <el-col :xs="24">
            <el-form-item>
              <el-button :disabled="loading.usuarios == true" type="success" icon="fas fa-save" @click="onClickSalvarEditar()"> Salvar Edição</el-button>
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      return {
        validaSenhaConfirmacaoFunc: validaSenhaConfirmacao,
        alterarSenha: false,
        ...
        validacoes: {
          ...
          username: [
            { required: true, validator: this.mxValidaUsernameUsuario, trigger: ['blur', 'submit'] }
          ],
          senha: [
            { required: false, validator: this.mxValidaSenhaUsuario, trigger: ['blur', 'submit'] }
          ],
          senhaConfirmacao: [
            { required: false, validator: validaSenhaConfirmacao, trigger: ['blur', 'submit'] }
          ],
          cpf: [
    ...
    async mounted() {
      ...
      this.paramsCidades.ufAbreviado = this.usuario.uf
    ...
    methods: {
      ...
      onAlterarSenha() {
        if (this.alterarSenha == true){
          this.formUsuario = { ...this.formUsuario, senhaAlterada: true, senha: '', senhaConfirmacao: '' }
          this.validacoes.senha[0].required = true
          this.validacoes.senha[0].validator = this.mxValidaSenhaUsuario
          this.validacoes.senhaConfirmacao[0].required = true
          this.validacoes.senhaConfirmacao[0].validator = this.validaSenhaConfirmacaoFunc
        }
        else {
          delete this.formUsuario.senha
          delete this.formUsuario.senhaConfirmacao
          this.formUsuario.senhaAlterada = false
          this.validacoes.senha[0].required = false
          this.validacoes.senha[0].validator = (rule, value, callback) => { return callback() }
          this.validacoes.senhaConfirmacao[0].required = false
          this.validacoes.senhaConfirmacao[0].validator = (rule, value, callback) => { return callback() }
        }
      },

      async onClickSalvarEditar() {
        this.loading.usuarios = true
        this.$refs.formUsuario.validate(async (valid) => {
          if (valid) {
            let { data, status } = await _api.usuarios.put(this.formUsuario)
            if (status === 200) {
              this.$emit('emit', 'editar-usuario-close')
            } else {
              this.$swal({
                title: "Erro!",
                text: 'Ocorreu um erro ao salvar a edição do usuário!',
                icon: 'error',
              })
            }
          } else {
