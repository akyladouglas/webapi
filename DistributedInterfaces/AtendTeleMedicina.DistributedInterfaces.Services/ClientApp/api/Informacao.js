...
export default {
  ...
  get (filters) {
    ...
    return axios({
      method: 'GET',
      data: filters,
      url: `${$config.urlApi}/InformacoesUteis`
    })
      .then(res => {
      ...
      .catch(e => {
        ...
        Notification({ title: 'Erro: ERRDBxpostINF01', message: erroMsg, type: 'error' })
  ...
  post (arr) {
    return axios({
      ...
      .catch(e => {
        ...
        Notification({ title: 'Erro: ERRDBxpostINF02', message: erroMsg, type: 'error' })
        return { status: 400, error: erroMsg }
      })
  },
  put (form) {
    return axios({
      method: 'PUT',
      data: form,
      url: `${$config.urlApi}/InformacoesUteis/${form.id}`
    })
      .then(res => {
        if (res.status === 200) Notification({ title: '', message: 'Informação excluida com sucesso!', type: 'success' })
        return res
      })
      .catch(e => {
