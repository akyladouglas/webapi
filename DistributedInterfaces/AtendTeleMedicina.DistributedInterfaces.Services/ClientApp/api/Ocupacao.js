...
export default {
  getByCbo(cbo) {
    return axios({
        method: 'GET',
        url: `${$config.urlApi}/Ocupacao/${cbo}`
      })
      .then(res => {
      ...
      .catch(e => {
        ...
        Notification({ title: 'Erro: ERRDBxgetCBO02', message: erroMsg, type: 'error' })
  ...
  get(filters) {
    return axios({
      ...
      .catch(e => {
        ...
        let erroMsg = e.response ? e.response.data.message : 'Erro de comunicação com a API'
        Notification({ title: 'Erro: ERRDBxgetCBO01', message: erroMsg, type: 'error' })
        return { status: 400, error: erroMsg }
      })
  }
}
