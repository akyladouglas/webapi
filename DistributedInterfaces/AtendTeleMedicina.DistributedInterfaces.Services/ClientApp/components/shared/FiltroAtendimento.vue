    ...
    <el-row :gutter="20">
      ...
      <el-col :xs="24" :sm="24" :md="12" :lg="6" :xl="6">
        <el-form-item label="CPF" prop="cpf">
          <el-input v-model="filtroParams.cpf" masked="true" maxlength="14" v-mask="'###.###.###-##'" />
        </el-form-item>
      </el-col>
    ...
    <el-row :gutter="20">
      ...
      <el-col :xs="24" :sm="24" :md="12" :lg="5" :xl="5">
        <el-form-item label="Classificação de Risco" prop="corStatus">
          <el-select v-model="filtroParams.corStatus" placeholder="Todos ..." clearable>
            <el-option v-for="item in enums.coresStatusAtendimento" :label="item.label" :value="item.value" :key="item.value" />
          </el-select>
        </el-form-item>
      </el-col>
      <el-col :xs="24" :sm="24" :md="12" :lg="5" :xl="5">
      <el-form-item label="Em Isolamento" prop="agravamento">
      ...
      <el-col :xs="24" :sm="24" :md="12" :lg="5" :xl="5">
        <el-form-item label="Último Atendimento" prop="dataUltimoAtendimento">
          <el-date-picker prefix-icon="fas fa-calendar-day" v-model="filtroParams.dataUltimoAtendimento" v-mask="`##-##-${moment().year()}`" type="date" format="dd-MM-yyyy" />
        </el-form-item>
      </el-col>
      <el-col :xs="24" :sm="24" :md="12" :lg="5" :xl="5">
        <el-form-item label="Grau de Prioridade" prop="grauDePrioridade">
          <el-select filterable placeholder="Todos" v-model="filtroParams.grauDePrioridade">
            <el-option v-for="option in enums.grausDePrioridade" :value="option.value" :label="option.label" :key="option.value" />
          </el-select>
        </el-form-item>
      </el-col>         
    </el-row>
...
export default {
    ...
    async mounted () {
      ...
      window.addEventListener('keyup', (e) => {
        if (e.keyCode === 13) {
          this.onClickFiltrar('filtroParams')
        }
      })
      await this.getUfs()
    },
    methods: {
      async onClickFiltrar (form) {
        ...
        this.$refs[form].validate((valid) => {
          if (valid) {
            this.$store.dispatch('setFilters', this.filtroParams)            
      ...
      async getUfs () {
        ...
        if (this.filtroParams.uf) {
          this.cidadeParams.uf = this.filtroParams.uf
        }
        this.getCidadesByUf()
