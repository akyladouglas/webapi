...
{
    public class InterconsultaCidRepository : BaseRepository, IInterconsultaCidRepository
    {
        private readonly StringBuilder _queryAll = new StringBuilder(
            @"SELECT 
                CAST(iCid.Id AS CHAR) AS Id,
                CAST(iCid.InterconsultaId AS CHAR) AS InterconsultaId,
                CAST(iCid.CidId AS CHAR) AS CidId,
                iCid.DataCadastro,
                iCid.DataAlteracao
                FROM InterconsultaCid iCid
            ");

        private readonly StringBuilder _queryCountAll = new StringBuilder(
            @"SELECT COUNT(iCid.Id) FROM InterconsultaCid iCid");

        private readonly MySqlConnection _connection;
        private readonly IUser _user;

        public InterconsultaCidRepository(IMySqlContext context, IUser user)
            : base(context)
        {
        ...
        public async Task<string> Post(InterconsultaCid cid)
        {
            ...
            paramInsert.Add("?Id", dbType: DbType.String, value: cid.Id, direction: ParameterDirection.Input);
            paramInsert.Add("?InterconsultaId", dbType: DbType.String, value: cid.InterconsultaId, direction: ParameterDirection.Input);
            paramInsert.Add("?CidId", dbType: DbType.Int32, value: cid.CidId, direction: ParameterDirection.Input);
            paramInsert.Add("?DataCadastro", dbType: DbType.DateTime, value: cid.DataCadastro, direction: ParameterDirection.Input);

            const string insertQuery = @"INSERT INTO InterconsultaCid (Id, InterconsultaId, CidId, DataCadastro)
                                                     VALUES (?Id, ?InterconsultaId, ?CidId, ?DataCadastro)";
            ...
            using (var transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    RegistrarAcao(_connection, Guid.NewGuid().ToString(), "InterconsultaCid", cid.Id.ToString(), "Inseriu a InterconsultaCid", DateTime.Now, _user.GetUserOrigem(), _user.GetUserId(), _user.GetUserIp());
                    transaction.Commit();
                    return cid.Id;
                }
                catch (Exception e)
