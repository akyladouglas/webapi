...
{
    public class InterconsultaRepository : BaseRepository, IInterconsultaRepository
    {
        private readonly StringBuilder _queryAll = new StringBuilder(
            @"SELECT 
                CAST(i.Id AS CHAR) AS Id,
                i.ClassificacaoDeRisco,
                i.DiscussaoDoCaso,
                i.DataCadastro,
                i.DataAlteracao
        ...
        private readonly StringBuilder _queryCountAll = new StringBuilder(
            @"SELECT COUNT(i.Id) FROM Interconsulta i");

        private readonly MySqlConnection _connection;
        private readonly IUser _user;

        public InterconsultaRepository(IMySqlContext context, IUser user)
            : base(context)
        {
        ...
        public async Task<string> Post(Interconsulta interconsulta)
        {
            ...
            paramInsert.Add("?Id", dbType: DbType.String, value: interconsulta.Id, direction: ParameterDirection.Input);
            paramInsert.Add("?ClassificacaoDeRisco", dbType: DbType.Int32, value: interconsulta.ClassificacaoDeRisco, direction: ParameterDirection.Input);
            paramInsert.Add("?DiscussaoDoCaso", dbType: DbType.String, value: interconsulta.DiscussaoDoCaso, direction: ParameterDirection.Input);
            paramInsert.Add("?DataCadastro", dbType: DbType.DateTime, value: interconsulta.DataCadastro, direction: ParameterDirection.Input);

            const string insertQuery = @"INSERT INTO Interconsulta (Id, ClassificacaoDeRisco, DiscussaoDoCaso, DataCadastro)
                                                     VALUES (?Id, ?ClassificacaoDeRisco, ?DiscussaoDoCaso, ?DataCadastro)";
            ...
            using (var transaction = _connection.BeginTransaction())
            {
                try
                {
                    ...
                    RegistrarAcao(_connection, Guid.NewGuid().ToString(), "Interconsulta", interconsulta.Id.ToString(), "Inseriu a Interconsulta", DateTime.Now, _user.GetUserOrigem(), _user.GetUserId(), _user.GetUserIp());
                    transaction.Commit();
                    return interconsulta.Id;
                }
                catch (Exception e)
                {
